...
func (c *Client) DeleteRequestBy(r *Request) (*ServeEvents, error) {
	requestBody, err := r.MarshalJSON()
	if err != nil {
		return nil, fmt.Errorf("delete request by: build error: %w", err)
	}

	res, err := http.Post(fmt.Sprintf("%s/%s/remove", c.url, wiremockAdminRequestsURN), "application/json", bytes.NewBuffer(requestBody))
	if err != nil {
		return nil, fmt.Errorf("delete request by: request error: %w", err)
	}
	defer res.Body.Close()

	bodyBytes, err := io.ReadAll(res.Body)
	if err != nil {
		return nil, fmt.Errorf("delete request by: read response error: %w", err)
	}
...
func (c *Client) FindRequestBy(r *Request) (*ServeEvents, error) {
	requestBody, err := r.MarshalJSON()
	if err != nil {
		return nil, fmt.Errorf("find request by: build error: %w", err)
	}

	res, err := http.Post(fmt.Sprintf("%s/%s/remove", c.url, wiremockAdminRequestsURN), "application/json", bytes.NewBuffer(requestBody))
	if err != nil {
		return nil, fmt.Errorf("find request by: request error: %w", err)
	}
	defer res.Body.Close()

	bodyBytes, err := io.ReadAll(res.Body)
	if err != nil {
		return nil, fmt.Errorf("get count requests: read response error: %w", err)
	}
	...
	err = json.Unmarshal(bodyBytes, &serveEvents)
	if err != nil {
		return nil, fmt.Errorf("get count requests: read json error: %w", err)
	}
	return &serveEvents, nil
}
